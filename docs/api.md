# Hodlong Documentation

Hodlong is a streaming torrent client for **Node.js** and the **web** that uses EOSIO Technologies. Hodlong
provides the same API in all environments.

To use WebTorrent in the browser, [WebRTC] support is required (Chrome, Firefox, Opera, Safari).

While the functionality of adding and managing torrents is the same as the [webtorrent] project, there are a few more required operators to connect to the eosio network.
...
var opts =
    {
      ...
      accountName: EOS_ACCOUNT_NAME,
      privatePassphrase: PRIVATE_HODLONG_PRIVATEKEY_PASSPHRASE,
      contractInfo:
            {
                'hodlong': EOS_HODLONG_CONTRACT,
                'trackers': EOS_TRACKER_CONTRACT
            }
      api: EOS_API // Can be the scatter, lynx, etc
      rpc: EOS_RPC // Also accepts the above wallets
    }
```
...
let opts = {
      dht: true,
      endpoint: 'http:////84ec401d.ngrok.io',
      accountName: 'usera',
