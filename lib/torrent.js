...
const debug = require('debug')('hodlong:torrent')
...
const utHodlong = require('ut_hodlong')
...
const USER_AGENT = `Hodlong/${VERSION} (https://hodlong.com)`
...
class Torrent extends EventEmitter {
  constructor (torrentId, client, opts) {
    ...
    this._paid = opts.paid === true // set to true if a superpeer
  ...
  // TODO: remove in v1
  get swarm () {
    console.warn('Hodlong: `torrent.swarm` is deprecated. Use `torrent` directly instead.')
    return this
  }
  ...
  _processParsedTorrent (parsedTorrent) {
    ...
    if (this.client.tracker && !this.private) {
      // So `hodlong-provider` can force specific trackers to be used
    }
  ...
  _onMetadata (metadata) {
    ...
    if (metadata && metadata.infoHash && metadata.chainId && metadata.objectId) {
  ...
  _addPeer (peer) {
    ...
    if (typeof peer === 'string' && this._paid && !this._validPubkey(peer)) {
      this._debug('ignoring peer: invalid pubkey %s', peer)
      return null
    }
  ...
  _onWire (wire, addr) {
    ...
    // use ut_hodlong extension
    wire.use(utHodlong(), this.hapi)
