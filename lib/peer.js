...
const debug = require('debug')('hodlong:peer')
const Wire = require('hodlong-protocol')
...
class Peer {
  ...
  onHandshake (infoHash, peerId) {
    if (!this.swarm){
      return // `this.swarm` not set yet, so do nothing
    }
    if (this.destroyed) return
  ...
  handshake () {
    const opts = {
      ...
      hodlongHandshake: this.swarm.hapi.getAccountName()
    }
